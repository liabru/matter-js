...
var f=e[p];if(f.isActive){var v=f.collision,y=v.parentA,m=v.parentB,g=v.normal,x=v.tangent,h=f.activeContacts,b=1/h.length;y.velocity.x=y.position.x-y.positionPrev.x,y.velocity.y=y.position.y-y.positionPrev.y,m.velocity.x=m.position.x-m.positionPrev.x,m.velocity.y=m.position.y-m.positionPrev.y,y.angularVelocity=y.angle-y.anglePrev,m.angularVelocity=m.angle-m.anglePrev;for(var S=0;S<h.length;S++){var C=h[S],w=C.vertex,A=r.sub(w,y.position,a),B=r.sub(w,m.position,l),P=r.add(y.velocity,r.mult(r.perp(A),y.angularVelocity),c),M=r.add(m.velocity,r.mult(r.perp(B),m.angularVelocity),d),k=r.sub(P,M,u),I=r.dot(g,k),T=r.dot(x,k),V=Math.abs(T),R=s.sign(T),E=(1+f.restitution)*I,_=s.clamp(f.separation+I,0,1)*n._frictionNormalMultiplier,L=T,O=1/0;V>f.friction*f.frictionStatic*_*o&&(O=V,L=s.clamp(f.friction*R*o,-O,O));var W=r.cross(A,g),F=r.cross(B,g),N=b/(y.inverseMass+m.inverseMass+y.inverseInertia*W*W+m.inverseInertia*F*F);if(E*=N,L*=N,0>I&&I*I>n._restingThresh*o)C.normalImpulse=0;else{var D=C.normalImpulse;
C.normalImpulse=Math.min(C.normalImpulse+E,0),E=C.normalImpulse-D}if(T*T>n._restingThreshTangent*o)C.tangentImpulse=0;else{var q=C.tangentImpulse;C.tangentImpulse=s.clamp(C.tangentImpulse+L,-O,O),L=C.tangentImpulse-q}i.x=g.x*E+x.x*L,i.y=g.y*E+x.y*L,y.isStatic||y.isSleeping||(y.positionPrev.x+=i.x*y.inverseMass,y.positionPrev.y+=i.y*y.inverseMass,y.anglePrev+=r.cross(A,i)*y.inverseInertia),m.isStatic||m.isSleeping||(m.positionPrev.x-=i.x*m.inverseMass,m.positionPrev.y-=i.y*m.inverseMass,m.anglePrev-=r.cross(B,i)*m.inverseInertia)}}}}}()},{"../core/Common":14,"../geometry/Bounds":24,"../geometry/Vector":26,"../geometry/Vertices":27}],11:[function(e,t,o){var n={};t.exports=n;var i=e("../geometry/Vertices"),r=e("../geometry/Vector");!function(){n.collides=function(t,n,s){var a,l,c,d,u=s,p=!1;if(u){var f=t.parent,v=n.parent,y=f.speed*f.speed+f.angularSpeed*f.angularSpeed+v.speed*v.speed+v.angularSpeed*v.angularSpeed;p=u&&u.collided&&.2>y,d=u}else d={collided:!1,bodyA:t,bodyB:n};if(u&&p){var m=d.axisBody,g=m===t?n:t,x=[m.axes[u.axisNumber]];
